...
delimiter byte is encountered. @code{until-byte*} is the same but a failure to find the delimiter is not a match failure. @code{length-prefixed} reads a 9P variable-length
field with a 2 byte big-endian length followed by that many bytes of data.
